...
const nutritionData: { [key: string]: { [item: string]: NutritionInfo } } = {
  ...
  proteinas: {
    ...
    'Langostinos al panko (+5.50)': { kcal: 145, proteina: 21, carbos: 8, fibra: 0 },
...
export default function PokeBowlBuilder({ pokeOptions }: PokeBowlBuilderProps) {
  ...
  const handleOrderClick = () => {
    ...
    const bowlProduct = {
      ...
      ingredients: [...(selectedItems.toppings || []), ...(selectedItems.agregados || [])],
    };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-0">
          ...
          <div className="p-8 lg:p-12 border-r border-gray-200 flex flex-col justify-between bg-linear-to-b from-white to-gray-50">
            <div>
              ...
              <p className="text-gray-600 text-sm leading-relaxed mb-8">
                Personaliza tu poke bowl seleccionando tus ingredientes favoritos. 
                Elige base, proteína, toppings, agregados y salsa para crear tu combinación perfecta. 
                Fresco, nutritivo y hecho a tu medida.
              </p>
                      ...
                      {isExpanded && (
                        <div className="mt-2 p-4 bg-white border-2 border-[#9CB973] rounded-xl">
                          <div className="space-y-2 max-h-48 overflow-y-auto">
                            {pokeOptions[key as keyof typeof pokeOptions].map((item) => {
            ...
            <div className="bg-[#9CB973]/10 border-2 border-[#9CB973] rounded-xl p-4 text-center">
              ...
              {selectedItems.bases?.length && selectedItems.proteinas?.length && (
              ...
              <button
                ...
                disabled={totalSelected === 0 || !selectedItems.bases?.length || !selectedItems.proteinas?.length}
                className={`w-full py-2 px-4 rounded-lg font-bold transition-all ${
                  totalSelected > 0 && selectedItems.bases?.length && selectedItems.proteinas?.length
          ...
          <div className="p-8 lg:p-12 flex flex-col items-center justify-center bg-linear-to-b from-white via-[#9CB973]/5 to-white relative overflow-hidden">
            ...
            <div className="mt-8 text-center z-10">
              <p className={`text-sm font-semibold ${selectedItems.bases?.length ? 'text-[#6B8E4E]' : 'text-gray-600'}`}>
                {selectedItems.bases?.length ? '✓ Base seleccionada' : 'Selecciona una base'}
              </p>
              <p className={`text-sm font-semibold mt-1 ${selectedItems.proteinas?.length ? 'text-[#6B8E4E]' : 'text-gray-600'}`}>
                {selectedItems.proteinas?.length ? '✓ Proteína seleccionada' : 'Selecciona una proteína'}
              </p>
            </div>
          </div>
          ...
          <div className="p-8 lg:p-12 border-l border-gray-200 bg-linear-to-b from-gray-50 to-white flex flex-col justify-between">
            <div>
              <p className="text-xs text-gray-500 uppercase tracking-widest font-bold mb-6">Este plato contiene</p>
              ...
              {totalSelected > 0 && (
                <div className="bg-[#9CB973]/10 rounded-lg p-4 mb-8 border border-[#9CB973]/20">
                  <p className="text-xs text-[#5D4E37] font-bold mb-3">📋 INGREDIENTES SELECCIONADOS</p>
                  <div className="space-y-2 text-xs text-gray-600">
                    {selectedItems.bases && selectedItems.bases.length > 0 && (
                      <p><span className="font-semibold">Base:</span> {selectedItems.bases[0]}</p>
                    )}
                    {selectedItems.proteinas && selectedItems.proteinas.length > 0 && (
                      <p><span className="font-semibold">Proteína:</span> {selectedItems.proteinas[0]}</p>
                    )}
                    {selectedItems.toppings && selectedItems.toppings.length > 0 && (
                      <p><span className="font-semibold">Toppings ({selectedItems.toppings.length}):</span> {selectedItems.toppings.join(', ')}</p>
                    )}
                    {selectedItems.agregados && selectedItems.agregados.length > 0 && (
                      <p><span className="font-semibold">Agregados ({selectedItems.agregados.length}):</span> {selectedItems.agregados.join(', ')}</p>
                    )}
                    {selectedItems.salsas && selectedItems.salsas.length > 0 && (
                      <p><span className="font-semibold">Salsas ({selectedItems.salsas.length}):</span> {selectedItems.salsas.join(', ')}</p>
                    )}
                  </div>
                </div>
              )}
              ...
              <div className="bg-[#F5F5F5] rounded-lg p-4 mb-8">
                <p className="text-xs text-gray-600 font-semibold">% NUTRIENTES DIARIOS</p>
                <p className="text-sm text-gray-500 mt-2">
                  Nuestros bowls contienen los nutrientes esenciales para una alimentación balanceada y deliciosa.
                </p>
              </div>
            </div>

            <div className="bg-[#9CB973]/10 border-2 border-[#9CB973] rounded-lg p-4 text-center">
              <p className="text-xs text-[#5D4E37] font-bold mb-2">💚 BOWL SALUDABLE</p>
              <p className="text-xs text-gray-600">Ingredientes frescos seleccionados con cuidado</p>
            </div>
          </div>
        </div>
      </div>
