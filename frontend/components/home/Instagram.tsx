...
export default function Instagram() {
  ...
  useEffect(() => {
    ...
    // Instagram embed.js reemplaza cada <blockquote> por un <iframe> sin title.
    // Usamos MutationObserver para asignar el title en cuanto aparezca el iframe.
    const observer = new MutationObserver(() => {
      const wrappers = document.querySelectorAll<HTMLElement>(".instagram-embed-wrapper");
      wrappers.forEach((wrapper, index) => {
        const iframe = wrapper.querySelector<HTMLIFrameElement>("iframe");
        if (iframe && !iframe.title) {
          iframe.title = `Publicación ${index + 1} de Instagram - Natural Bowls (@naturalbowls.cafe)`;
        }
      });
    });
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <a
            ...
            className="inline-flex items-center gap-2 bg-[#4D7A30] hover:bg-[#3E6B22] text-white px-8 py-3 rounded-lg font-semibold transition-all duration-300"
          >
